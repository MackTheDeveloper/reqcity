                ...
                <div class="col-12 col-sm-12 col-md-8 col-lg-9">
                    <div class="right-sides-items">
                        <div class="mysubscription-page">
                                        ...
                                        <div class="col-md-12">
                                            <div class="boxlayout-subscriptions">
                                                ...
                                                <div class="current-subplan-price">
                                                    <h4>${{ $currentSubscription->amount }}</h4>
                                                    <span>/{{ $currentSubscription->plan_type }}</span>
                                                </div>
                                                ...
                                                @if ($upcoming)
                                                    @if ($upcoming['activated'] != '2')
                                                        ...
                                                        <form class="submitPageForm" data-type="cancel-scheduled"
                                                            method="POST"
                                                            action="{{ route('SubscriptionPlanCancelSchedule') }}">
                                                            @csrf
                                                            <button class="border-btn">Cancel Subscription</button>
                                                        </form>
                                                    @endif
                                                @else
                                                    @if (!$isCancelledSubscription && $currentSubscription)
                                                        <form class="submitPageForm" data-type="cancel-current"
                                                            method="POST" action="{{ route('SubscriptionPlanCancel') }}">
                                                            @csrf
                                                            <button class="border-btn">Cancel Subscription</button>
                                                        </form>
                                                    @endif
                            ...
                            @if((!$upcoming || $upcoming['activated']=='1') && !$isCancelledSubscription)
                                <div class="upgrade-subscription-box">
                                    ...
                                    @else
                                        <p class="upgrade-plan-title">Would you like to downgrade
                                            your plan?</p>
                                    @endif
                                    ...
                                    @endif
                                    <form class="submitPageForm" data-type="{{$chooseSubscription['plan_type']=='yearly' ? 'upgrade' : 'downgrade'}}" method="POST"
                                        action="{{ route('SubscriptionPlanUpgrade') }}">
                                        @csrf
                                        <button class="fill-btn">Choose
                                            {{ ucfirst($chooseSubscription['plan_type']) }}</button>
                                    </form>
                                </div>
                            @endif
                        </div>
