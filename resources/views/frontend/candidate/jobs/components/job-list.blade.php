    ...
    @foreach ($jobListData as $jobs)
        <div class="job-posts-row">
            <div class="job-post-data">
                <a href="{{ route('showJobDetails', $jobs['jobSlug']) }}">
            ...
            <div class="job-post-status">
                @if ($jobs['isApplied'] != 1)
                    <form data-job-id="{{ $jobs['jobId'] }}" class="applyNowCandidate" data-type="downgrade">
                ...
                @endif
                <label class="bk">
                    <input data-job-id="{{ $jobs['jobId'] }}" class="makeFavourite" type="checkbox"
        ...
        @include('frontend.components.delete-confirm', [
            'title' => 'Confirm',
