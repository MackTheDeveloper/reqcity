        ...
        <div class="app-main__inner">
                            ...
                            <div class="page-title-subheading opacity-10">
                                <nav class="" aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item">
                                            <a href="javascript:void(0)" style="color: grey">Users</a>
                                        </li>
                                        <li class="breadcrumb-item">
                                            <a href="{{url(config('app.adminPrefix').'/user/list')}}" style="color: grey">List</a>
                                        </li>
                                        <li class="active breadcrumb-item" aria-current="page">
                                            <a href="#" style="color: slategray">Add</a>
                                        </li>
                                    </ol>
                                </nav>
                            </div>
            ...
            <div class="main-card mb-3 card">
                <div class="card-body">
                    ...
                    @endif
                    <form id="addNewUser" method="post" action="{{url(config('app.adminPrefix').'/user/add')}}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div>
                                        <input type="email" pattern="^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)+$" required class="form-control" id="email" name="email" placeholder="Enter email" value="{{ old('email') }}" />
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="select_role">Role <span class="text-danger">*</span></label>
                                    <select name="select_role" id="select_role" class="multiselect-dropdown form-control" data-width="100%">
                                        <optgroup label="Select Role">
                                            @foreach($roles as $role)
                                            @if($role->role_type != "super_admin")
                                            <option value="{{$role->id}}" {{ old('select_role') ? "selected" : "" }}>{{$role->role_title}}</option>
                                            @endif
                                            @endforeach
                                        </optgroup>
                                    </select>
                        ...
                        <div class="form-group">
                            ...
                            <a href="{{url(config('app.adminPrefix').'/user/list')}}"><button type="button" class="btn btn-light" name="cancel" value="Cancel">Cancel</button></a>
                        </div>
                    </form>
                </div>
            </div>
