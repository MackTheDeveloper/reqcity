...
$("#addJobFieldOptionForm").validate({
...
$(document).ready(function(){
    ...
    $('#Tdatatable').on('click', 'tbody .job_field_option_delete', function () {
        var job_field_option_id = $(this).data('id');
        var message = "Are you sure ?";
        console.log(message);
        $('#JobFieldOptionDeleteModel').on('show.bs.modal', function(e){
            $('#job_field_option_id').val(job_field_option_id);
            $('#message_delete').text(message);
        });
        $('#JobFieldOptionDeleteModel').modal('show');
    })

    $(document).on('click','#deleteJobFieldOption', function(){
        var job_field_option_id = $('#job_field_option_id').val();
        $.ajax({
            url: origin + '/../delete/' + job_field_option_id,
            method: "POST",
            data: {
                "_token": $('#token').val(),
                job_field_option_id: job_field_option_id,
            },
            success: function(response)
            {
                if(response.status == 'true')
                {
                    $('#JobFieldOptionDeleteModel').modal('hide')
                    DatatableInitiate();
                    toastr.clear();
    ...
    $('#Tdatatable').on('click', 'tbody .toggle-is-active-switch', function () {
        ...
        $('#JobFieldOptionIsActiveModel').on('show.bs.modal', function(e){
            $('#job_field_option_id').val(job_field_option_id);
            $('#status').val(status);
            $('#message').text(message);
        });
        $('#JobFieldOptionIsActiveModel').modal('show');
    });


    /** Activate or deactivate music cateogry */
    $(document).on('click','#JobFieldOptionIsActive', function(){
        var job_field_option_id = $('#job_field_option_id').val();
        ...
        $.ajax({
            ...
            success: function(response)
            {
                if(response.status == 'true')
                {
                    $('#JobFieldOptionIsActiveModel').modal('hide')
                    DatatableInitiate();
                    toastr.clear();
                    toastr.options.closeButton = true;
                    toastr.success(response.msg);
...
function DatatableInitiate() {
    $('#Tdatatable').DataTable(
        {
            language: {
                searchPlaceholder: "Search by option,job field... "
            },
            "oSearch": {"sSearch": $('#searchJobFieldOption').val() },
            "bDestroy": true,
            "processing": true,
            "serverSide": true,
